...
主要功能入口，基于 Flask + Leaflet.js + Chart.js 的五视图交互查看器。左侧固定侧边栏通过图标切换四个顶层视图。
...
| 折线图 | 按指标（速度 / 心率 / 功率 / 踏频 / 海拔 / 坡度）分别绘图；x 轴支持按公里 / 每 100 m / 累计时间切换；仅展示该文件有数据的指标 |
...
| AI 训练状态分析 | 基于当前 PMC 数据的流式 AI 点评 |

**训练日历：** 按月展示每日骑行活动，点击可查看当日详情。
...
Web 查看器内置两个 AI 功能：**单次骑行评估**（骑行详情视图点击「AI 评估」）和 **PMC 训练状态分析**（分析视图 PMC 标签页）。两者均需要配置一个 OpenAI 兼容的 API。
