...
主要功能入口，基于 Flask + Leaflet.js + Chart.js 的多视图交互查看器。左侧固定侧边栏通过图标切换五个顶层视图。
...
| 地图（卡片按钮） | 每条卡片附带「地图」按钮，点击清空当前地图轨迹、仅加载该条路径并切换到地图视图 |
| 打开地图（列表按钮） | 骑行记录标题栏的「打开地图」按钮，仅切换到地图视图，不加载任何文件 |
